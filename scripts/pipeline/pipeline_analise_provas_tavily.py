#!/usr/bin/env python3
"""
Pipeline de Análise de Provas — MedMind  [VARIANTE: TAVILY]
=============================================================
Busca de comentários via Tavily Web Search API (Fase 3).

Fluxo completo em 7 etapas:

  1. Lê o arquivo JSON de provas produzido pelo crawler
  2. Constrói queries otimizadas por prova
  3. Busca comentários via Tavily em paralelo (10 threads)  ← Tavily
...
Comparação com a variante Gemini (pipeline_analise_provas_gemini.py):
...
Uso básico:
  python pipeline_analise_provas_tavily.py caminho/para/provas.json
...
Opções úteis:
  ...
  --output PASTA       Pasta de saída (padrão: dados/pipeline_output_tavily)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Pipeline MedMind [Tavily]: Tavily Search → Gemini Filter → Claude Batches",
...
Variáveis de ambiente necessárias:
  ...
  ANTHROPIC_API_KEY   https://platform.anthropic.com
    ...
    parser.add_argument(
        "--output",
        default="dados/pipeline_output_tavily",
        help="Pasta de saída (padrão: dados/pipeline_output_tavily)",
    )
    parser.add_argument(
        "--limite",
        type=int,

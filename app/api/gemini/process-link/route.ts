  ...
  if (result.tokenizationData && result.chunking) {
    try {
      processingRunId = await persistProcessingPipeline({
        ...
        sourceName: downloaded.filename,
        extractionText: result.wholeExtractionText ?? result.originalText ?? result.text,
        processedText: result.transformedText ?? result.text,
        wholeExtractionText: result.wholeExtractionText ?? result.originalText ?? result.text,
        cleanedExtractionText: result.transformedText ?? result.text,
        extractionMetadata: {
          url,
          mimeType,
          ...
          saida_extracao_pos_limpeza: result.saida_extracao_pos_limpeza,
        },

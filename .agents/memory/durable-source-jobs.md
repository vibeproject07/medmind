...
**How to apply:** Use compare-and-swap por claim/run em transições, rollback se o claim for perdido, mantenha somente um run atual por fonte e coordene a escolha global de jobs no banco. Após persistir chunks, reivindique e vetorize o run exato no mesmo ciclo; todas as entradas, inclusive rotas diretas, devem usar claim/lease. Vetorize somente blocos `chunk` com texto não vazio, sem modificar seu conteúdo; reconcilie runs terminais com a fonte após interrupções.